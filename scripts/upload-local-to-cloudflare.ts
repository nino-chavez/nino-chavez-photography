...
async function uploadFileToCF(filePath: string, imageId: string): Promise<CFUploadResponse> {
...
async function main() {
	...
	async function worker() {
		while (index < toUpload.length) {
			...
			if (DRY_RUN) {
				console.log(`   [DRY] Would upload: ${photo.file} → ${photo.imageKey}`);
				uploaded++;
				continue;
			}

			try {
				const result = await uploadFileToCF(photo.path, photo.imageKey);
				...
				if (result.success && result.result) {
					...
					// Already exists in CF - just update the DB
					const { error: updateError } = await supabase
						.from('photo_metadata')
						.update({ cf_image_id: photo.imageKey })
						.eq('image_key', photo.imageKey)
						.eq('album_key', albumKey);

					if (!updateError) alreadyExists++;
					else failed++;
				} else {
