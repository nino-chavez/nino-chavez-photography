...
- **Remaining drops — each has a LIVE tendril, so they are a dedicated migration, not leaf drops:**
  - **6 vanity columns** (`composition`/`lighting`/`color_temperature`/`time_of_day`/`emotion`/`action_intensity`): `match_photos` (3 live callers: search, chat, related-photos) RETURNS 4 of them; `normalize_composition` (dead maint fn) refs `composition`. Drop = recreate `match_photos` without those output cols + verify the 3 callers don't read them + drop `normalize_composition` + drop the 6 cols.
