...
3. **Faces: self-hosted InsightFace vs hosted (Replicate/serverless GPU) — recommend hosted.** Identical accuracy per TASKS.md. Self-hosting is the single heaviest ops component (always-on GPU box, second security surface, non-Pages deploy target) on an otherwise fully-serverless stack run by a part-time operator. **Recommendation: hosted, pay-per-invocation, for both the backfill and incremental passes; self-host only if a measured cost/latency need later justifies the ops burden (write the "why not hosted" sentence then).**
