...
Canonical design tokens for the nino-chavez-photography family — OKLCH color, type scale, spacing, motion. Framework-agnostic; consumed by React (`packages/ui`), Svelte (`apps/storefront-svelte`), vanilla HTML (`apps/demos`), and any future surface that joins the family.
...
.brand-tint {
  ...
  background: oklch(from var(--nino-brand-lch) 0.97 0.03 h);
}
```

Change `--nino-brand-lch` once and every derived shade re-themes automatically. The same idiom is used in BigCommerce's storefront-kit; we intentionally rhyme.
...
.headline {
  color: oklch(var(--nino-foreground-lch));
  font-family: var(--nino-font-heading);
  font-size: 2.5rem;
}
```
...
All variables are prefixed `--nino-*` so they never collide with merchant-supplied tokens on the storefront, BigDesign tokens in the admin marketplace shell, or BC platform tokens elsewhere.
