<script lang="ts">
	...
	let userName = $state('');
	let consentObtained = $state(false);
...
<div class="bg-white rounded-lg shadow-sm border border-gray-200 p-4">
	...
	{#if showForm}
		...
		>
			...
			<input type="hidden" name="consentObtained" value={consentObtained.toString()} />
			...
			<div class="flex items-start gap-2">
				<input
					type="checkbox"
					id="consentObtained"
					bind:checked={consentObtained}
					class="w-4 h-4 mt-1 border border-gray-300 rounded focus:ring-2 focus:ring-blue-500 cursor-pointer"
				/>
				<label for="consentObtained" class="text-xs text-gray-600 cursor-pointer">
					I have permission from the athlete (or their parent/guardian if under 18) to tag them.
				</label>
			</div>
			...
			<div class="flex gap-2">
				<button
					type="submit"
					disabled={isSubmitting || !athleteName || !consentObtained}
